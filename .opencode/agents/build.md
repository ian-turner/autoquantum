...
- `.opencode/agents/<name>.md` files are the single source of truth for agent configuration; editing them takes effect on next OpenCode startup with no other changes needed
...
| Task | Delegate to |
|------|-------------|
| Writing or filling Lean proof bodies | `@prove` |
| Fetching and analyzing arXiv papers | `@read` |
| Translating Lean proofs to LaTeX | `@latex` |
