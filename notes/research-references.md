...
- **Relevance:** Most direct starting point for our formalization. Examine before duplicating.
...
- **Relevance:** Good reference for how to handle quantum information at the Lean 4 / Mathlib interface.
...
- **Relevance:** Demonstrates Lean is capable of sophisticated quantum physics results; useful for understanding what Mathlib infrastructure is available.
...
- **Relevance:** Closest prior work to AutoQuantum's approach. Study their feedback loop design.
...
- **Relevance:** Fine-tuning approach complementary to our verification approach; could provide a good generator to pair with the Lean checker.
...
- **Summary:** Embeds QWIRE quantum circuit language in Coq with HOAS, linear wire types, and denotational semantics as superoperators on density matrices.
- **Relevance:** Foundational patterns for circuit semantics; the inQWIRE group later created LeanQuantum building on this experience.
...
- **Relevance:** Advanced program logic for quantum programs; reference for how to reason about quantum program correctness structurally.
...
- **Relevance:** Most directly relevant to our QFT correctness proof. The abstraction technique may be adaptable to reduce the Lean proof burden.
...
- Mathlib `LinearAlgebra.Matrix.DFT` — discrete Fourier transform matrix (check if present)
