...
| `Algorithms/HPlus.lean` | No | Uniform superposition |+⟩^⊗n scaffolding added April 19, 2026; the algorithm file now owns `hPlusVector`, `hPlusVector_norm`, and `hPlusState`, keeping `Core/Hilbert.lean` focused on shared foundational definitions; remaining HPlus gaps are `tensorVec_norm`/`tensorState` support and `hPlus_correct`. |
