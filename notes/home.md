...
| `Algorithms/GHZ.lean` | No | General `n`-qubit GHZ development added on April 18, 2026. The file now defines `zeroIndex`, `onesIndex`, `prefixOnesIndex`, `allZeroState`, `allOneState`, `prefixOnesState`, `ghzState`, `ghzProgressState`, the nearest-neighbor `ghzCnotChain`, and the general `ghzCircuit`; it also proves the 1-qubit base case via `hadamardAt_fin1_zero`, `ghzState_one_eq_ketPlus`, `apply_hadamard_allZero_one`, and `ghzCircuit_prepares_ghz_zero`. Only the general theorem `ghzCircuit_prepares_ghz` remains a `sorry`; the remaining work is now the two transition lemmas that connect the circuit to `ghzProgressState` |
