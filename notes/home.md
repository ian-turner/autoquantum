...
| `Algorithms/GHZ.lean` | No | General `n`-qubit GHZ development added on April 18, 2026. The file now defines `zeroIndex`, `onesIndex`, `prefixOnesIndex`, `allZeroState`, `allOneState`, `prefixOnesState`, `ghzState`, `ghzProgressState`, the nearest-neighbor `ghzCnotChain`, and the general `ghzCircuit`; it also proves the 1-qubit base case via `hadamardAt_fin1_zero`, `ghzState_one_eq_ketPlus`, `apply_hadamard_allZero_one`, and `ghzCircuit_prepares_ghz_zero`. The latest pass also added `lowBitIndex`, `lowBitIndex_val`, `qubitPerm_zeroIndex`, `qubitPerm_lowBitIndex`, and `apply_permuteQubits_allZero`, plus `runCircuit_append` in `Lemmas/Circuit.lean`, so the remaining work is now concentrated in the general Hadamard-step lemma and the CNOT-chain induction for `ghzCircuit_prepares_ghz` |
